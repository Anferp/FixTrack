<template>
  <div class="dashboard-layout">
    ...
    <div class="dashboard-content">
      <h1 class="dashboard-title">Dashboard de Secretaria</h1>
      ...
      <div class="orders-container">
        ...
        <div v-else-if="orders.length === 0" class="no-orders">
          <span class="material-icons no-orders-icon">inbox</span>
          <p>No hay órdenes disponibles</p>
          <router-link to="/secretary/orders/new" class="create-order-button">
          ...
          >
            <template #actions>
              <div class="order-actions">
                <router-link :to="`/secretary/orders/${order.id}`" class="view-order-button">
                  Ver detalles
                </router-link>
              </div>
            </template>
...
async function fetchStatusDistribution() {
  try {
    ...
    if (data.success) {
      ...
      pendingOrders.value = distribution.find(item => item.status === 'pending')?.count || 0;
      inReviewOrders.value = distribution.find(item => item.status === 'in_review')?.count || 0;
      completedOrders.value = distribution.find(item => item.status === 'repaired')?.count || 0;
    } else {
      throw new Error(data.error || 'Error al obtener la distribución de estados');
    }
  } catch (err) {
    console.error('Error fetching status distribution:', err);
    // No establecemos error.value aquí para no interferir con la visualización de órdenes
  }
}
...
.create-order-button .material-icons {
  margin-right: 8px;
}
